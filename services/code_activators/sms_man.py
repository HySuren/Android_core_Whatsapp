...
class SmsMan(BaseActivator):

    NAME = ActivatorName.SMS_MAN
        ...
        while True:
            try:
                response = requests.post(
                    ...
                    params={
                        "api_key": KeyActivator.SMS_MAN,
                        "action": "getNumber",
                        "service": srvice,
                        "country": REGIONS.get_code(self.region, activator_name='sms_activate'),
                    },
                )
    ...
    def get_code(self, id: int, timeout=60):
        ...
        while True:
            response = requests.post(
                "http://api.sms-man.ru/stubs/handler_api.php?",
                params={
                    "api_key": KeyActivator.SMS_MAN,
                    "action": "getStatus",
                    "id": id
