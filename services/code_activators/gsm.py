...
from .config import ActivatorName, ActivatorTimeout, REGIONS


class Gsm(BaseActivator):

    NAME = ActivatorName.GSM
        ...
        while True:
            try:
                response = requests.get(
                    url="https://smsapi.gsm2.biz/api1.php?",
                    params={
                        "key": 'Wy2G0sAW1JchPjwi1y1q',
                        "action": "getNumber",
                        "service": srvice,
    ...
    def get_code(self, id: int, timeout=60):
        ...
        while True:
            response = requests.get(
                "https://smsapi.gsm2.biz/api1.php?",
                params={
                    "key": 'Wy2G0sAW1JchPjwi1y1q',
                    "action": "getStatus",
                    "id": id
        ...
        code = response.text.split(':')[1].replace("'", "").replace('-', '')
        return code
    ...
    def phone_cancel(self, id: int):
        self._update_number_status(id, status=13)
        return True
