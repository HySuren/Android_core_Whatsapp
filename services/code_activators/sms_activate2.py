...
class SmsActivate2(BaseActivator):
    ...
    def get_phone(self, obj, srvice='wa', free_price=False, max_free_price=None, free_price_array=None):
        ...
        while True:
            try:
                if free_price and len(free_price_array) > 0:
                    for price in free_price_array:
                        ...
                        obj.logger.info(f'sms_activate2 request end with {response.status_code} status_code: {response.text}')
                    ...
                    response = requests.post(
                        ...
                        params={
                            "api_key": KeyActivator.SMS_ACTIVATE2,
                            "action": "getNumberV2",
                            "service": srvice,
            ...
            if obj.sms_activate_response_enable:
                if response.status_code >= 500:
                    obj.device_info.params['sms_activate_response']['responses'].append({"status_code": response.status_code, "response_text": "HTML_ERROR", 'service': 'sms_activate2'})
                else:
                    obj.device_info.params['sms_activate_response']['responses'].append({"status_code": response.status_code, "response_text": response.text, 'service': 'sms_activate2'})
        ...
        if obj.sms_activate_response_enable:
            obj.logger.info(f'logging sms_activate2 answer')
